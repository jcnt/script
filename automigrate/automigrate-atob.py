...
response = faa.post_pods(pod=pod, names=[POD])
print("Creating Pod...", response.status_code)

container = {"pod": {"name": POD}}
response = faa.patch_volumes(names=[VOL], volume=container)
print("Moving volume to Pod...", response.status_code)

response = faa.post_pods_arrays(member_names=["PureCZ-FA-B"], group_names=[POD])
print("Adding Pod member...", response.status_code)

podvol = POD + "::" + VOL
response = fab.get_volumes(names=[podvol])
...
while resp != 200:
    print(".", end="", flush=True)
    time.sleep(1)
    response = fab.post_connections(volume_names=[podvol], host_names=["jjRHEL"])
